...
def get_article_url_config(url_id):
    ...
    cursor.execute(
        "Select tag_name,scrape_type,attribute_name from article_url_configuration where domain_url_id = '%s'" % url_id)
...
def get_article_publish_date_config(url_id):
    ...
    # Extracting all article url configuration details from database against url
    cursor.execute(
        "Select tag_name,scrape_type,attribute_name from article_publish_date_configuration where domain_url_id = '%s'" % url_id)
...
def get_article_text_config(url_id):
    ...
    # Extracting all article url configuration details from database against url
    cursor.execute(
        "Select tag_name,scrape_type,attribute_name from article_text_configuration where domain_url_id = '%s'" % url_id)
...
def get_article_topic_head_config(url_id):
    ...
    # Extracting all article url configuration details from database against url
    cursor.execute(
        "Select parent_tag_name,child_tag_name,scrape_type,attribute_name from article_topic_headline_configuration where domain_url_id = '%s'" % url_id)
    ...
    for url_id in range(len(url_details_lst)):  # get all url one by one
        try:
            ...
            for article_url in article_url_extracted_lst:  # get all article url from domain url one by one
                ...
                doc = BeautifulSoup(driver.page_source, "html.parser")
                driver.quit()
                remove_tag = ['header', 'script', 'noscript', 'footer', "button", "input", "style", "hr", "br",
                              "label", "nav", "form",  'meta', 'fieldset', "ins", "style"]
                ...
                # database connection
                if (merged_data != "") or (merged_data is not None) or (article_headline != '') or (article_headline is not None):
                    connection = pymysql.connect(host="localhost", user="root", passwd="q1w2e3rty12345",
                                                 database="automated_news_broadcast")
                    cur = connection.cursor()
                    # inserting article url into database
                    cur.execute(
                        "Insert into unprocesssed_scrape_data (article_id,unprocessed_news_topic,unprocessed_news_description,publication_date,"
                        "image_href,scrape_date_stamp,scrape_time_stamp) "
